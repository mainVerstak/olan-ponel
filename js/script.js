$(document).ready(function () {
  ...
  $(".ds-save-search").on("click", function (event) {
    var $_SELF = $(this);
    event.preventDefault();
    $_SELF.addClass("opacity-animate");
    setTimeout(() => {
      $_SELF.removeClass("opacity-animate");
    }, 300);
  });
...
$(function () {
  ...
  if ($(".js-bank-select").length > 0) {
    $.fn.select2.amd.define(
      "select2/HideSelectedResultsAdapter",
      ["select2/utils", "select2/results"],
      function (Utils, ResultsList) {
        var HideSelectedResultsAdapter = function (
          decorated,
          $element,
          options,
          dataAdapter
        ...
        HideSelectedResultsAdapter.prototype.append = function (
          decorated,
          data
        ) {
    ...
    var CloseOnSelect = $.fn.select2.amd.require(
    ...
    $(".js-bank-select").each((_i, e) => {
      ...
      $e.select2({
        ...
        var searchfield = $(this)
          .parent()
          .find("textarea.select2-search__field");
  ...
  $(document).on("click", ".js-open-edit-feed-input", function () {
    ...
    $(this)
      .closest(".feed-input")
      .find(".feed-input__input")
      .addClass("_last-edit");
    let currentFeed = $(this)
      .closest(".feed-input")
      .find(".feed-input__input")
      .attr("href");
    $(".modal-edit-feed-input").val(currentFeed);
    $("#modal-edit-feed .modal-feed-form-validation")
      .data("bootstrapValidator")
      .resetForm();
  });
  $(document).on("click", ".js-open-add-feed-input", function () {
    $(".modal-new-feed-input").val("");
    $("#modal-add-feed .modal-feed-form-validation")
      .data("bootstrapValidator")
      .resetForm();
  });
  ...
  $(".js-add-feed-input").on("click", function (e) {
    e.preventDefault();
    $(this)
      .closest(".modal-feed-form-validation")
  ...
  $(document).on("click", ".table-mortgage2__btn-detail", function () {
    ...
    if ($content.hasClass("_active")) {
      $item
        .find(".table-mortgage2__btn-detail")
        .removeClass("_active")
  ...
  $(window).on("resize", function () {
    ...
    if ($(window).width() < 992) {
      $(".calc-mortgage-new__bottom-btns").appendTo(
        $(".mortgage-mobile-btn-wrapper")
      );
    } else {
      $(".calc-mortgage-new__bottom-btns").appendTo(
        $(".calc-mortgage-new__bottom")
      );
    }
  });
  ...
  if ($(".show-table-tariff-popover").length > 0) {
    $(".show-table-tariff-popover")
      .popover()
      .map(function () {
        $(this).data("bs.popover").tip().addClass("table-tariff-popover");
      });
  }
  ...
  $(".js-mortgage-more").on("click", function () {
    ...
    if ($container.hasClass("active")) {
      $container.removeClass("active");
      $(".btn.js-mortgage-more")
        .find(".btn-styled-new2__inner")
        .html(
          '<i class="fa fa-plus"></i>' +
  ...
  if ($(".calc-mortgage-new").length > 0) {
    ...
    //clear on focus
    $("#calc-mortgage-summ, #calc-mortgage-initial-fee").on(
      "focus",
      function () {
        $(this).val(
          $(this)
    ...
    $(".calc-mortgage-new #calc-mortgage-initial-fee").on(
      "focusout",
      function (e) {
        var summ = Number(
            $(this)
    ...
    function sumVal() {
      var initialFee = $("#calc-mortgage-initial-fee")
          .val()
          .replace(/[^0-9]/g, ""),
        years = $("#calc-mortgage-term")
          .val()
          .replace(/[^0-9]/g, ""),
        price = $("#calc-mortgage-summ")
          .val()
          .replace(/[^0-9]/g, "");
      $(".calc-mortgage-new__sum").html(
        Math.round(((price - initialFee) / (years * 12)) * 1.09)
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " ₽"
      );
    }
